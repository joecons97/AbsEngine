            ...
            if (chunk.TerrainVertices != null && chunk.TerrainVertices.Count > 0)
            {
                if (opaqueJob == null)
                    opaqueJob = _renderJobs
                        .FirstOrDefault(x => x.Layer == ChunkRenderLayer.Opaque 
                                        && x.IsChunkValidForRegion(chunk) 
                                        && x.HasSpaceFor(chunk.TerrainVertices.Count))
            ...
            if (chunk.WaterVertices != null && chunk.WaterVertices.Count > 0)
            {
                if (transparentJob == null)
                    transparentJob = _renderJobs
                        .FirstOrDefault(x => x.Layer == ChunkRenderLayer.Transparent
                                        && x.IsChunkValidForRegion(chunk)
                                        && x.HasSpaceFor(chunk.WaterVertices.Count))
